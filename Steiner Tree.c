/* 
 * Steiner Tree.
 * We need to form an MST "T" that contains all the given terminal vertices.
...
{ 
    ...
    /*
     * Finding the Non-Terminal vertices
     */
    int r = 0;				/* r --> Number of Non-Terminal Vertices */	
    for(i=1;i<V;i++)
    {
		int flag = 0;
		for(j=0;j<K;j++)
		{
		    if(i == terminal_vertices[j])
		    {
				flag = 1;
				break;
		    }
		...
		if(flag == 0)
		{
		    remaining_vertices[r] = i;
		    r++;
		}
    }

    printf("Given Terminal Vertices ---> ");
    for(i=0;i<K;i++)
    {
		printf("%d  ",terminal_vertices[i]);
    }
    printf("\nNon-Terminal Vertices   ---> ");
    for(i=0;i<r;i++)
    {
		printf("%d  ",remaining_vertices[i]);
    }
     ...
     */
    printf("\nTerminal Vertex \"%d\" is added to T\n",terminal_vertices[0]);
